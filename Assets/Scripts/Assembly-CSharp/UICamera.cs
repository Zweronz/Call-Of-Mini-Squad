...
public class UICamera : MonoBehaviour
{
	...
	protected static void SetSelection(GameObject go, ControlScheme scheme)
	{
		...
		else
		{
			...
			{
				UICamera uICamera = FindCameraForLayer(mCurrentSelection.layer);
				if (uICamera != null)
				{
					current = uICamera;
			...
			if (list.size > 0)
			{
				UICamera uICamera2 = ((!(mNextSelection != null)) ? list[0] : FindCameraForLayer(mNextSelection.layer));
				if (uICamera2 != null)
				{
	...
	public static bool Raycast(Vector3 inPos, out RaycastHit hit)
	{
		for (int i = 0; i < list.size; i++)
		{
			...
			if (uICamera.eventType == EventType.World)
			{
			...
			else if (uICamera.eventType == EventType.UI)
			{
			...
			else if (uICamera.eventType == EventType.Unity2D)
			{
				Collider2D collider2D = Physics2D.OverlapPoint(vector, layerMask);
				if ((bool)collider2D)
				{
					hit = lastHit;
					hit.point = vector;
	...
	private void Update()
	{
		...
		if (mCurrentSelection != null)
		{
			...
			if (text.Length > 0)
			{
				...
				Notify(mCurrentSelection, "OnInput", text);
			}
		}
		else
		{
	...
	public void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		else if (currentTouch.pressed != null && (currentTouch.delta.magnitude != 0f || currentTouch.current != currentTouch.last))
		{
			...
			else if (!currentTouch.dragStarted && num < magnitude)
			{
				flag2 = true;
				currentTouch.dragStarted = true;
				currentTouch.delta = currentTouch.totalDelta;
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.dragged == currentTouch.current || (currentScheme != ControlScheme.Controller && currentTouch.clickNotification != 0 && currentTouch.totalDelta.magnitude < num))
			{
				if (currentTouch.pressed != mCurrentSelection || currentTouch.pressed.GetComponent<UIInput>() != null)
				{
					mNextSelection = null;
