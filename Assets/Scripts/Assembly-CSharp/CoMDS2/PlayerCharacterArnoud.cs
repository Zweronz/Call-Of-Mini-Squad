...
namespace CoMDS2
{
	public class PlayerCharacterArnoud : Player
	{
		private SoldierEmplacement m_emplacement;
		...
		public override void Initialize(GameObject prefab, string name, Vector3 position, Quaternion rotation, int layer)
		{
			...
			DataConf.SkillArnoud skillArnoud = (DataConf.SkillArnoud)DataCenter.Conf().GetHeroSkillInfo(base.characterType, base.playerData.skillLevel, base.playerData.skillStar);
			base.skillInfo = skillArnoud;
			m_skillTotalCDTime = base.skillInfo.CDTime;
			m_emplacement = new SoldierEmplacement(this);
			GameObject prefab2 = Resources.Load<GameObject>("Models/ActiveObjects/Emplacement/Emplacement");
			m_emplacement.Initialize(prefab2, "SoldierEmplacement", Vector3.zero, Quaternion.identity, 0);
